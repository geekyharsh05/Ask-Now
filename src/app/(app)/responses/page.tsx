...
export default function ResponsesPage() {
  ...
  const { data: surveyResponses, isLoading: surveyResponsesLoading } =
    useResponsesBySurvey(selectedSurveyId || 0);
  ...
  const responsesLoading = selectedSurveyId
    ...
    responses?.filter((response) => {
      const matchesSearch =
        ...
        (response.isAnonymous &&
          "anonymous".includes(searchTerm.toLowerCase()));
  ...
  const handleExportResponses = () => {
    ...
    const csvRows = filteredResponses.map((response) => {
      const baseRow = [
        ...
        format(
          new Date(response.completedAt || response.createdAt),
          "yyyy-MM-dd HH:mm:ss"
        ),
        response.ipAddress || "",
      ];
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">
              {Math.round(
                (anonymousResponses.length / (filteredResponses.length || 1)) *
                  100
              )}
              % of total
            </p>
      ...
      <Tabs defaultValue={urlTab} className="space-y-4">
                    ...
                    {filteredResponses.map((response) => (
                      <TableRow key={response.id}>
                        ...
                        <TableCell>
                          <Badge
                            variant={
                              response.isAnonymous ? "secondary" : "default"
                            }
                          >
                            {response.isAnonymous ? "Anonymous" : "Registered"}
                          </Badge>
        ...
        <TabsContent value="anonymous" className="space-y-4">
          <Card>
            <CardHeader>
              <CardTitle>
                Anonymous Responses ({anonymousResponses.length})
              </CardTitle>
              <CardDescription>
      ...
      <Dialog open={isDetailOpen} onOpenChange={setIsDetailOpen}>
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              Detailed view of the survey response
            </DialogDescription>
          </DialogHeader>
          {selectedResponse && (
